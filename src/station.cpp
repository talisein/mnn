...
namespace mnn
{
  ...
  {
    if (is_assistant_emergency_coordinator == m.is_assistant_emergency_coordinator) {
      return; // No change
    }
  ...
  Station::vfunc_set_location(double latitude, double longitude) noexcept
  {
    if (m.location.has_value()) {
      if (latitude != SHUMATE_MIN_LATITUDE || longitude != SHUMATE_MIN_LONGITUDE) {
        if (latitude != m.location->latitude || longitude != m.location->longitude) {
